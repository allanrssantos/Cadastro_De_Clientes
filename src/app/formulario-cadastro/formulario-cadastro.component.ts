...
export class FormularioCadastroComponent implements OnInit {
  ...
  private carregarCadastro() {
    if (this.currentAction == 'edit') {
      this.route.paramMap
        .pipe(
          switchMap((params) =>
            this._clientesService.buscarPorId(+params.get('id'))
          )
        )
        .subscribe(
          (c) => {
  ...
  private criarCadastro(cadastroForm) {
    const clienteTela: Clientes = Object.assign(
      new Clientes(),
      cadastroForm.value
    );
    this._clientesService.gravar(clienteTela).subscribe(
  ...
  private atualizarCadastro(cadastroForm) {
    ...
    this._clientesService.atualizar(clienteTela).subscribe(
      () => this.cadastroGravadoComSucesso(),
      (error) => this.erroAoGravarCadastro(error)
    );
  }
  ...
  private atualizarPageTitle() {
    if (this.currentAction == 'new') this.pageTitle = 'Novo Cadastro';
    else {
